...
- **2026-03-31 — Builder UX Phase 8:** Centralize subtree duplication as `cloneUiSubtreeWithNewIds` (new ids at every node); call `sanitizeSelection` after `removeNode` so the selection store cannot reference a deleted id. Keep keyboard handlers scoped to “not typing in inputs” to avoid fighting form fields. Drive starter layouts via `BUILDER_DOCUMENT_TEMPLATES` and render template buttons from that array so adding new templates stays a metadata-only change.

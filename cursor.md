...
- **2026-03-31 - React Flow custom nodes + edges:** Edges between **custom** `nodeTypes` need **`Handle`** nodes (source/target); otherwise React Flow logs **#008** (“Couldn’t create edge for source handle id: null”). Add handles on the left/right (or top/bottom) to match the tree layout direction.
- **2026-03-31 - Builder primary surface:** The **runtime `BuilderCanvas`** (layout overlay + `RuntimePreviewHost`) is **removed**; the **page graph** (`PageFlowCanvas` + `ShadcnNodePreview`) is the only main workspace. Full **pixel/runtime parity** is validated via **`/preview`** (same DSL → `RuntimeSurface`), not by embedding runtime in the graph.
- **2026-03-31 - Page graph palette + previews:** Wire palette drops with **`CanvasDropData`** (`parentId` + `depth`); use a full-pane droppable at **`depth: -1`** so **`canvasPointerCollision`** still prefers deeper node targets. Keep the source of truth as `UiNode` data and render via **`ShadcnNodePreview`** instead of storing DOM nodes or prebuilt component instances.

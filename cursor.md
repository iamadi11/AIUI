...
- **2026-03-31 — Multi-select ergonomics:** Add explicit `setSelection(ids)` in `selection-store` for bulk operations and drive **Select All** with Cmd/Ctrl+A in `builder-demo` using a tree walk. Keep root selectable but non-destructive actions (`remove`/`duplicate`) still skip root.
- **2026-03-31 — Range select in tree:** Implement Shift+Click range selection in `NodeTree` using preorder ids from `collectNodeIds(root)`; use the primary selected id as the anchor and `setSelection(slice)` for deterministic contiguous ranges.
- **2026-03-31 — Parent navigation shortcut:** Support Alt+ArrowUp in `builder-demo` to move selection to the current node’s parent (`findParentOf` against `document.root`); keep it no-op at root and when nothing is selected.

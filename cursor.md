...
- **2026-03-31 - Page graph palette + previews:** Wire palette drops with the same **`CanvasDropData`** shape as **`BuilderCanvas`**; use a full-pane droppable at **`depth: -1`** so **`canvasPointerCollision`** still prefers deeper node targets. For “show components, not cards”, keep the source of truth as `UiNode` data and render via a React preview mapper (`ShadcnNodePreview`) instead of storing DOM nodes or prebuilt component instances.

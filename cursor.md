...
- **2026-03-31 — runtime-core flush:** Only re-seed `state` from `doc.state` on initial mount and `update()` (`shouldResetStateFromDoc`); action-driven `rebuild` keeps `state`. When `prevConfigRef === config` and listeners exist, skip `replaceChildren` and only run `layoutDocument` + rect patches (`patchSubtree`).

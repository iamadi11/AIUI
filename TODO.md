...
- **2026-03-31** — **Builder reliability follow-up:** effect sync in `DocumentStatePanel` and `EventBindingsPanel` now depends on source values (`document.state`, `events`) instead of only serialized fingerprints, removing stale-hook lint warnings and keeping undo/import refresh deterministic.
- **2026-03-31** — **Builder power follow-up:** added `setSelection(ids)` in `selection-store` and wired **⌘/Ctrl+A** to select all layers from the current document tree (`collectNodeIds` in `builder-demo`); shortcuts help updated.
- **2026-03-31** — **Builder selection follow-up:** tree supports **Shift+Click** range select (document preorder) between primary selection and clicked node; shortcuts help + README updated.
- **2026-03-31** — **Builder navigation follow-up:** added **Alt+↑** to select parent of the current primary selection (`findParentOf` in `builder-demo`); shortcuts help + README updated.

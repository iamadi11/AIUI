...
- [ ] **Phase 3 — Layout UX (remaining):** Richer snapping; per-side margin in Properties; constraint UI
...
- **2026-03-31** — **Product UX — Phase 3 layout follow-up (partial):** `layout.margin` in `@aiui/layout-engine` (`parseMargin`, column + row spacing); registry + Properties **Margin (px)**; canvas **resize handle** on empty leaves (8px snap, min 32px) → `layout.width` / `layout.height`; **resize alignment guides**; Vitest `layout.margin.test.ts`
